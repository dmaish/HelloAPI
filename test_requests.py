...
class BookListApiTestcase(unittest.TestCase):
    ...
    def test_get_book_by_id(self):
        ...
        access_token = self.get_access_token(self.user_data)
        get_res = self.client.get('/api/books/{}'.format(self.book["id"]), headers={
                                        'content-type': 'application/json',
                                        'Authorization': 'Bearer {}'.format(access_token)
                                    })
        self.assertEqual(get_res.status_code, 200)
    ...
    def test_can_edit_book(self):
        ...
        access_token = self.get_access_token(self.user_data)
        post_res = self.client.post('/api/books/', data=self.book, headers={
                                        'content-type': 'application/json',
                                        'Authorization': 'Bearer {}'.format(access_token)
                                    })
        self.assertEqual(post_res.status_code, 201)
        put_res = self.client.put('/api/books/{}'.format(self.book["id"]), data=json.dumps({'title': 'Inferno'}), headers={
                                        'content-type': 'application/json',
                                        'Authorization': 'Bearer {}'.format(access_token)
                                    })
