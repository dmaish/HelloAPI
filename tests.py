...
class BookListApi(unittest.TestCase):
    ...
    def test_new_book_creation(self):
        ...
        post_res = self.client().post('/books/', data=self.book)
    ...
    def test_get_book_by_id(self):
        ...
        get_res = self.client().get('/books/{}'.format(json_response['id']))
        self.assertEqual(get_res.status_code, 200)
        self.assertIn('The Da Vinci Code', str(get_res.data))
    ...
    def test_can_edit_book(self):
        ...
        post_res = self.client().post('/books/', data=self.book)
        self.assertEqual(post_res.status_code, 201)
        put_res = self.client().put('/books/1',data={'title': 'inferno'})
    ...
    def test_book_deletion(self):
        ...
        post_res = self.client().post('/books/', data=self.book)
        self.assertEqual(post_res.status_code, 201)
        del_res = self.client().delete('/books/1')
